...
Rts2Connection::Rts2Connection (Block * in_master):Connection (in_master)
{
	sock = -1;
	buf_top = buf;
	full_data_end = NULL;
...
Rts2Connection::Rts2Connection (int in_sock, Block * in_master):Connection (in_master)
{
...
Rts2Connection::~Rts2Connection (void)
{
	if (sock >= 0)
		close (sock);
	delete serverState;
...
int Rts2Connection::add (Block *block)
{
	if (sock >= 0)
	{
...
std::string Rts2Connection::getCameraChipState (int chipN)
{
...
std::string Rts2Connection::getStateString ()
{
	std::ostringstream _os;
...
void Rts2Connection::postEvent (Event * event)
{
	if (otherDevice)
...
void Rts2Connection::postMaster (Event * event)
{
	master->postEvent (event);
}

int Rts2Connection::idle ()
{
	if (connectionTimeout > 0)
	{
		...
		{
			logStream (MESSAGE_DEBUG) << "Rts2Conn timeout: " << lastGoodSend
...
int Rts2Connection::authorizationOK ()
{
...
int Rts2Connection::authorizationFailed ()
{
...
void Rts2Connection::checkBufferSize ()
{
...
int Rts2Connection::acceptConn ()
{
	...
	if (new_sock == -1)
	{
		logStream (MESSAGE_ERROR) << "Rts2Connection::acceptConn data accept " << strerror (errno) << sendLog;
		return -1;
	}
	else
	{
		...
		sock = new_sock;
		#ifdef DEBUG_EXTRA
		logStream (MESSAGE_DEBUG) << "Rts2Connection::acceptConn connection accepted" << sendLog;
		#endif
...
void Rts2Connection::setState (rts2_status_t in_value, char * msg)
{
...
void Rts2Connection::setBopState (rts2_status_t in_value)
{
...
void Rts2Connection::setOtherType (int other_device_type)
{
	delete otherDevice;
...
int Rts2Connection::getOtherType ()
{
	if (otherDevice)
		return otherType;
	return -1;
...
void Rts2Connection::updateStatusWait (Rts2Connection * conn)
{
	if (runningCommand && runningCommand->getStatusCallProgress () == CIP_WAIT)
		sendCommand ();
}

void Rts2Connection::masterStateChanged ()
{
	if (runningCommand && runningCommand->getStatusCallProgress () == CIP_RUN)
		sendCommand ();
}

void Rts2Connection::processLine ()
{
	...
	#ifdef DEBUG_ALL
	std::cout << "Rts2Conn::processLine [" << getCentraldId ()
...
bool Rts2Connection::receivedData (Block *block)
{
	return block->isForRead (sock);
}

void Rts2Connection::processBuffer ()
{
	if (full_data_end)
		return;
...
int Rts2Connection::receive (Block *block)
{
	int data_size = 0;
	...
	if ((sock >= 0) && (block->getPollEvents (sock) & (POLLIN | POLLPRI)))
	{
		...
		successfullRead ();
		#ifdef DEBUG_ALL
		std::cout << "Rts2Connection::receive name " << getName ()
			<< " [" << getCentraldId () << ":" << sock << "]"
			<< " reas: " << buf_top
			<< " full_buf: " << buf
			<< " size: " << data_size
...
int Rts2Connection::writable (Block *block)
{
	if (sock >=0 && (block->getPollEvents (sock) & POLLOUT) && isConnState (CONN_INPROGRESS))
	{
		...
		if (ret)
		{
			logStream (MESSAGE_ERROR) << "Rts2Connection::idle getsockopt " <<
...
int Rts2Connection::getOurAddress (struct sockaddr_in *in_addr)
{
...
void Rts2Connection::setAddress (struct in_addr *in_address)
{
	addr = *in_address;
}

void Rts2Connection::getAddress (char *addrBuf, int _buf_size)
{
	char *addr_s;
...
void Rts2Connection::setCentraldId (int in_centrald_id)
{
	centrald_id = in_centrald_id;
}

void Rts2Connection::queCommand (Command * cmd, int notBop, Object * originator)
{
...
int Rts2Connection::queCommand (Command * cmd)
{
	#ifdef DEBUG_ALL
	std::cout << "Rts2Connection::queCommand " << cmd->getText () << " runningCommand " << runningCommand << " queu size " << commandQue.size () << std::endl;
	#endif
...
void Rts2Connection::queSend (Command * cmd)
{
	cmd->setConnection (this);
	if (isConnState (CONN_CONNECTING)
		|| isConnState (CONN_INPROGRESS)
...
void Rts2Connection::commandReturn (Command * cmd, int in_status)
{
	if (otherDevice)
		otherDevice->commandReturn (cmd, in_status);
}

bool Rts2Connection::queEmptyForOriginator (Object *testOriginator)
{
...
void Rts2Connection::queClear (bool running)
{
...
void Rts2Connection::deleteConnection (Rts2Connection *conn)
{
...
int Rts2Connection::command ()
{
	...
	logStream (MESSAGE_DEBUG) << "Rts2Connection::command unknow command: getCommand " << getCommand () << " state: " << conn_state << " type: " << getType () << " name: " << getName () << sendLog;
	sendCommandEnd (DEVDEM_E_COMMAND, ss.str ().c_str ());
	return -4;
}

int Rts2Connection::statusProgress ()
{
	long long int value;
	char *msg = NULL;
	...
	if (paramNextLongLong (&value) || paramNextDouble (&statusStart) || paramNextDouble (&statusExpectedEnd) || !(paramEnd() || (paramNextString (&msg) == 0 && paramEnd ())))
		return -2;
	//std::cerr << "Rts2Connection::statusProgress " << value << " " << statusStart << " " << statusExpectedEnd << std::endl;	
	setState (value, msg);
	master->progress (this, statusStart, statusExpectedEnd);
	return -1;
}

int Rts2Connection::status ()
{
	long long int value;
	char *msg = NULL;
	if (paramNextLongLong (&value) || !(paramEnd () || (paramNextString (&msg) == 0 && paramEnd ())))
		return -2;
	//std::cerr << "Rts2Connection::status " << value << std::endl;	
	setState (value, msg);
	return -1;
}

int Rts2Connection::bopStatus ()
{
	...
	if (paramNextLongLong (&masterStatus) || paramNextLongLong (&masterBopState) || !paramEnd ())
		return -2;
	//std::cerr << "Rts2Connection::bopStatus " << masterStatus << " " << masterBopState << std::endl;	
	setBopState (masterBopState);
	setState (masterStatus, NULL);
	return -1;
}

int Rts2Connection::message ()
{
...
void Rts2Connection::sendCommand ()
{
...
int Rts2Connection::sendNextCommand ()
{
	// pop next command
...
int Rts2Connection::commandReturn ()
{
	int ret;
	...
	if (!runningCommand)
	{
		logStream (MESSAGE_ERROR) << "Rts2Connection::commandReturn null on connection with '"
			<< getName ()
			<< "' (" << getCentraldId ()
			<< ") and status " << stat
			<<"!" << sendLog;
	...
	#ifdef DEBUG_ALL
	std::cout << "Rts2Connection::commandReturn " << ret << std::endl;
	#endif
	switch (ret)
	{
...
int Rts2Connection::sendMsg (const char *msg)
{
	int len;
	int ret;
	if (sock == -1)
	{
		#ifdef DEBUG_ALL
		std::cout << "Rts2Connection::sendMsg sock -1" << std::endl;
		#endif
		return -1;
	}
	len = strlen (msg) + 1;
	...
	strcpy (mbuf, msg);
	#ifdef DEBUG_ALL
	std::cout << "Rts2Connection::sendMsg will send " << msg << std::endl;
	#endif
	strcat (mbuf, "\n");
	// ignore EINTR
	do
	...
	if (ret != len)
	{
		...
		logStream (MESSAGE_ERROR)
			<< "Rts2Connection::sendMsg [" << getCentraldId () << ":" << conn_state << "] error "
	...
	#ifdef DEBUG_ALL
	std::cout << "Rts2Connection::sendMsg " << getName ()
...
int Rts2Connection::sendMsg (std::string msg)
{
	return sendMsg (msg.c_str ());
}

int Rts2Connection::sendMsg (std::ostringstream &_os)
{
	return sendMsg (_os.str ().c_str ());
}

int Rts2Connection::startBinaryData (int dataType, int channum, size_t *chansize)
{
	std::ostringstream _os;
	dataConn++;
...
int Rts2Connection::sendBinaryData (int data_conn, int chan, char *data, size_t dataSize)
{
...
void Rts2Connection::endBinaryData (int data_conn)
{
	std::ostringstream _os;
...
int Rts2Connection::startSharedData (DataSharedWrite *data, int channum, int *segnums)
{
	std::ostringstream _os;
	dataConn++;
...
void Rts2Connection::endSharedData (int data_conn, bool complete)
{
	std::ostringstream _os;
	if (complete)
	{
...
int Rts2Connection::fitsDataTransfer (const char *fn)
{
	std::ostringstream _os;
	chmod (fn, 0666);
...
void Rts2Connection::successfullSend ()
{
	time (&lastGoodSend);
}

void Rts2Connection::getSuccessSend (time_t * in_t)
{
	*in_t = lastGoodSend;
}

bool Rts2Connection::reachedSendTimeout ()
{
	time_t now;
	time (&now);
...
void Rts2Connection::successfullRead ()
{
	time (&lastData);
}

void Rts2Connection::connConnected ()
{
}

void Rts2Connection::connectionError (int last_data_size)
{
	activeReadData = -1;
	if (canDelete ())
...
int Rts2Connection::sendMessage (Message & msg)
{
	return sendMsg (msg.toConn ());
}

int Rts2Connection::sendValue (std::string val_name, int value)
{
	std::ostringstream _os;
...
int Rts2Connection::sendValue (std::string val_name, int val1, double val2)
{
	std::ostringstream _os;
...
int Rts2Connection::sendValue (std::string val_name, const char *value)
{
...
int Rts2Connection::sendValueRaw (std::string val_name, const char *value)
{
	if (getConnState () == CONN_INPROGRESS)
	{
		return -1;
...
int Rts2Connection::sendValue (std::string val_name, double value)
{
	std::ostringstream _os;
	_os.setf (std::ios_base::fixed, std::ios_base::floatfield);
	_os.precision (20);
...
int Rts2Connection::sendValue (char *val_name, char *val1, int val2)
{
	std::ostringstream _os;
...
int Rts2Connection::sendValue (char *val_name, int val1, int val2, double val3, double val4, double val5, double val6)
{
	std::ostringstream _os;
	_os.setf (std::ios_base::fixed, std::ios_base::floatfield);
	_os.precision (20);
...
int Rts2Connection::sendValueTime (std::string val_name, time_t * value)
{
	std::ostringstream _os;
	_os << PROTO_VALUE " " << val_name << " " << *value;
	return sendMsg (_os);
...
int Rts2Connection::sendProgress (double start, double end)
{
	std::ostringstream _os;
...
int Rts2Connection::sendCommandEnd (int num, const char *in_msg)
{
	std::ostringstream _os;
	if (num == 0)
		_os << "+0";
...
void Rts2Connection::setConnState (conn_state_t new_conn_state)
{
...
int Rts2Connection::isConnState (conn_state_t in_conn_state)
{
	return (conn_state == in_conn_state);
}

bool Rts2Connection::paramEnd ()
{
...
int Rts2Connection::paramNextString (char **str, const char *enddelim)
{
	while (isspace (*command_buf_top))
		command_buf_top++;
	if (!*command_buf_top)
...
char * Rts2Connection::paramNextWholeString ()
{
	while (isspace (*command_buf_top))
		command_buf_top++;
	return command_buf_top;
...
int Rts2Connection::paramNextStringNull (char **str)
{
	int ret;
...
int Rts2Connection::paramNextInteger (int *num)
{
	char *str_num;
	char *num_end;
...
int Rts2Connection::paramNextLong (long int *num)
{
	char *str_num;
	char *num_end;
	if (paramNextString (&str_num, ","))
...
int Rts2Connection::paramNextLongLong (long long int *num)
{
	char *str_num;
	char *num_end;
	if (paramNextString (&str_num, ","))
...
int Rts2Connection::paramNextSizeT (size_t * num)
{
	char *str_num;
	char *num_end;
	if (paramNextString (&str_num))
...
int Rts2Connection::paramNextSSizeT (ssize_t * num)
{
	char *str_num;
	char *num_end;
	if (paramNextString (&str_num))
...
int Rts2Connection::paramNextDouble (double *num)
{
	char *str_num;
	int ret;
	if (paramNextString (&str_num, ","))
...
int Rts2Connection::paramNextDoubleTime (double *num)
{
	char *str_num;
	int ret;
	if (paramNextString (&str_num, ","))
...
int Rts2Connection::paramNextFloat (float *num)
{
	char *str_num;
	int ret;
	if (paramNextString (&str_num, ","))
...
int Rts2Connection::paramNextHMS (double *num)
{
	char *str_num;
	if (paramNextString (&str_num))
		return -1;
...
int Rts2Connection::paramNextDMS (double *num)
{
	char *str_num;
	if (paramNextString (&str_num))
		return -1;
...
int Rts2Connection::paramNextTimeval (struct timeval *tv)
{
	int sec;
	int usec;
...
void Rts2Connection::newDataConn (int data_conn)
{
	if (otherDevice)
		otherDevice->newDataConn (data_conn);
}

void Rts2Connection::dataReceived ()
{
...
Value * Rts2Connection::getValue (const char *value_name)
{
	return values.getValue (value_name);
}

Value * Rts2Connection::getValueType (const char *value_name, int value_type)
{
...
ValueVector::iterator & Rts2Connection::getFailedValue (ValueVector::iterator &iter)
{
...
void Rts2Connection::addValue (Value * value, const ValueVector::iterator eiter)
{
...
int Rts2Connection::metaInfo (int rts2Type, std::string m_name, std::string desc)
{
...
int Rts2Connection::selMetaClear (const char *value_name)
{
...
int Rts2Connection::selMetaInfo (const char *value_name, char *sel_name)
{
	Value *val = getValue (value_name);
	if (!val || val->getValueType () != RTS2_VALUE_SELECTION)
		return -2;
...
const char * Rts2Connection::getValueChar (const char *value_name)
{
	Value *val;
...
double Rts2Connection::getValueDouble (const char *value_name)
{
	Value *val;
	val = getValue (value_name);
	if (val)
...
int Rts2Connection::getValueInteger (const char *value_name)
{
	Value *val;
	val = getValue (value_name);
	if (val)
...
const char * Rts2Connection::getValueSelection (const char *value_name)
{
	Value *val;
	val = getValue (value_name);
...
const char * Rts2Connection::getValueSelection (const char *value_name, int val_num)
{
	Value *val;
	val = getValue (value_name);
...
int Rts2Connection::commandValue (const char *v_name)
{
...
bool Rts2Connection::existWriteType (int w_type)
{
...
double Rts2Connection::getProgress (double now)
{
...
DataAbstractRead* Rts2Connection::lastDataChannel (int chan)
{
...
double Rts2Connection::getInfoTime ()
{
	if (info_time)
...
void Rts2Connection::getInfoTime (struct timeval &tv)
{
	if (info_time)
	{
...
bool Rts2Connection::infoTimeChanged ()
{
	if (info_time)
	{
