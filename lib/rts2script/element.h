...
/**
 * Sequence of observations. Parsed from seq string, which specifies filter,
 * exposure duration and number of exposures.
 *
 * @author Petr Kubanek <petr@kubanek.net>
 */
class ElementSeq:public Element
{
	public:
		ElementSeq (Script * _script, float in_expTime);
		virtual int nextCommand (rts2core::DevClientCamera * camera, rts2core::Command ** new_command, char new_device[DEVICE_NAME_SIZE]);

		virtual void prettyPrint (std::ostream &os) { os << "sequence of " << repeat << " " << exptime << " seconds exposures in filter " << filter; }
		virtual void printXml (std::ostream &os) { os << "  <seq filter='" << filter << "' exptime=" << exptime << " repeat=" << repeat << "/>"; }
		virtual void printScript (std::ostream &os) { os << COMMAND_SEQ " " << filter << " " << exptime << " " << repeat; }
		virtual void printJson (std::ostream &os) { os << "\"cmd\":\"" << COMMAND_SEQ << "\",\"filter\":\"" << filter << "\",\"exptime\":" << exptime << ",\"repeat\":" << repeat; }
