...
int Image::evalAF (float *result, float *error)
{
	...
	median = classicMedian (imd, npixels, &sigma);
	free (imd);
	#ifdef VERBOSE
	printf ("median %2.2lf +- %2.2lf\n", median, sigma);
	#endif
	findStar (_data);
	...
	for (i = 0; i < (int) list.size (); i++)
	{
		...
		printf ("%d %d\n", tmp.x, tmp.y);
		#endif
		aperture (_data, tmp, &ret);
		centroid (_data, ret, &px, &py);
		r = radius (_data, px, py, 6);
		integrate (_data, px, py, r, &xx);
		#ifdef VERBOSE
		printf ("%3.1lf %3.1lf %d %3.1lf\n---------------\n", px, py, r, xx);
		#endif
		vysledky.push_back (xx);
		vysledek += xx;
	}
	vysledek /= list.size ();

	for (i = 0; i < (int) vysledky.size (); i++)
	{
		chyba += (vysledek - vysledky[i]) * (vysledek - vysledky[i]);
	}
	#ifdef VERBOSE
	printf ("%2.2lf %2.2lf\n", vysledek, chyba / vysledky.size ());
	#endif
	*result = vysledek;
	*error = chyba / vysledky.size ();
