...
class ChunkResponse(http.client.HTTPResponse):
    ...
    def __init__(self, sock, debuglevel=0, strict=0, method=None, buffering=False):
        ...
        try:
            ...
            http.client.HTTPResponse.__init__(self, sock, debuglevel=debuglevel, method=method, buffering=buffering)
        except TypeError as te:
            #http.client.HTTPResponse.__init__(self, sock, debuglevel=debuglevel, strict=strict, method=method)
            http.client.HTTPResponse.__init__(self, sock, debuglevel=debuglevel, method=method)
...
class Rts2JSON:
    def __init__(self,url='http://localhost:8889',username=None,password=None,verbose=False,http_proxy=None):
        ...
        self.db = __db(self)
        self.headers = {'Authorization':'Basic' + base64.encodestring('{0}:{1}'.format(username,password).encode()).decode().replace('\n','')}#.strip()}
        self.prefix = prefix
    ...
    def getResponse(self, path, args=None, hlib=None):
        ...
        try:
            ...
            if not(r.status == http.client.OK):
                ...
                raise Exception(http.client.responses[r.status])
            return r
    ...
    def loadJson(self, path, args=None):
        ...
        #print ('CONT: {}'.format(str(d)))
        #print ('TYPE: {}'.format(type(d)))
        return sysjson.loads(str(d,'utf-8'))
...
class JSONProxy(Rts2JSON):
    ...
    def getDevice(self, device, refresh_not_found = False):
        self.refresh()
        try:
            #print('CONT:  ', self.devices)
            #print('DEVICE:  ', device)
            #print('TYPE:  ', type(self.devices))
            #print('TYPE:  ', type(self.devices[device]))
            return self.devices[device]
        except KeyError as ke:
            #if refresh_not_found == False:
            #    raise ke
            #was self.refresh(device)
            #return self.devices[device]
            self.refresh(device)
            return self.devices[device]
