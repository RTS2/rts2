...
class TestConfiguration(unittest.TestCase):
    ...
    def setUp(self):
        self.cf = Configuration(logger=logger)
        self.fileName='/usr/local/etc/rts2/rts2saf/rts2saf.cfg'
        self.success=self.cf.readConfiguration(fileName=self.fileName)

    #@unittest.skip('feature not yet implemented')
    def test_readConfiguration(self):
        self.assertTrue(self.success, 'config file: {} faulty or not found'.format(self.fileName))

        self.assertIs(type(self.cf), Configuration)
# ToDo: this fails
#        self.assertIs(type(self.cf.ccd), CCD)
        self.assertIsInstance(self.cf.ccd, CCD)
        self.assertIsInstance(self.cf.foc, Focuser)
        self.assertIsInstance(self.cf.filterWheels[0], FilterWheel)
        self.assertIsInstance(self.cf.filterWheels[0].filters[0], Filter)


    def test_checkConfiguration(self):
        self.assertTrue(self.success, 'config file: {} faulty or not found'.format(self.fileName))
        result = self.cf.checkConfiguration()
