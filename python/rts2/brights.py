...
def find_stars_on_data(data, verbose=0, useDS9=False):
    ...
    logging.debug('global average background: %.2f rms: %.3f threshold: %.3f',
                  bkg.globalback, bkg.globalrms, thres)
...
def get_brightest(s_objects, fn, verbose=0, useDS9=False, exclusion=None, pix_treshold=None):
    ...
    logging.debug('detected %d objects', len(s_objects))
            ...
            for j in range(i, len(s_objects)):
                if i != j:
                    ...
                    if dist(s_x, s_y, b_x, b_y) < exclusion:
                        logging.debug(
                            'rejecting star at %f %f, too close to %f %f', b_x, b_y, s_x, s_y)
                        break
    ...
    logging.info('brightest at %.2f %.2f', b_x, b_y)
    logging.debug('x     y     flag npix')
    for o in s_objects:
        logging.debug('%5.1f %5.1f %3d %4d %4d',
                      o['x'], o['y'], o['flag'], o['tnpix'], o['npix'])
        i += 1
    ...
    if useDS9:
        ...
        try:
            ...
            try:
                ...
                pass
        d.set('regions', 'image; point({0},{1}) # point=cross 25, color=green'.format(
            b_x, b_y))
        if verbose > 1:
            ...
            for o in s_objects[1:]:
                ...
                w = 20 * w
                w = w if w > 1 else 1
                d.set('regions', 'image; point({0},{1}) # point=cross {2},color=green'.format(
                    o['x'], o['y'], int(w)))
...
def add_wcs(fn, asecpix, rotang, flip='', verbose=0, dss=False, useDS9=False, outfn='out.fits', save_regions=None, center=None):
    ...
    try:
        ...
        logging.debug('cannot find DER1.PA, using defaults')
    ...
    w.wcs.cd = (asecpix / 3600.0) * np.array([[flip_x * rt_cos, flip_x * rt_sin], [
        flip_y * -rt_sin, flip_y * rt_cos]], np.float)
    w.wcs.ctype = ['RA---TAN', 'DEC--TAN']

    pixcrd = np.array([[x, y], [x + 1, y + 1], [0, 0]], np.float)
    logging.debug('some pixels %s', w.all_pix2world(pixcrd, 1))
    ...
    if center is None:
        offsp = np.array([[x, y], [hdu[0].header['NAXIS1'] /
                                   2.0, hdu[0].header['NAXIS2'] / 2.0]], np.float)
    else:
