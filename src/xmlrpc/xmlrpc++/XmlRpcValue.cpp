...
namespace XmlRpc
{
  ...
  void XmlRpcValue::invalidate ()
  {
	...
	break;
	case TypeDateTime:delete _value.asTime;
	break;
	case TypeBase64:delete _value.asBinary;
	break;
	case TypeArray:delete _value.asArray;
	break;
	case TypeStruct:delete _value.asStruct;
	break;
	default:break;
      }
    _type = TypeInvalid;
  ...
  void XmlRpcValue::assertArray (int in_size) const
  {
    if (_type != TypeArray)
      throw XmlRpcException ("type error: expected an array");
    else
  if (int (_value.asArray->size ()) < in_size)
    throw XmlRpcException ("range error: array index too large");
  }


  void XmlRpcValue::assertArray (int in_size)
  {
    if (_type == TypeInvalid)
      {
	_type = TypeArray;
	_value.asArray = new ValueArray (in_size);
      }
    else if (_type == TypeArray)
      {
	if (int (_value.asArray->size ()) < in_size)
	  _value.asArray->resize (in_size);
      }
    else
      throw XmlRpcException ("type error: expected an array");
  }
  ...
  {
    char buf[256];
      snprintf (buf, sizeof (buf) - 1, getDoubleFormat ().c_str (),
		_value.asDouble);
      buf[sizeof (buf) - 1] = 0;
  ...
  bool XmlRpcValue::stringFromXml (std::string const &valueXml, int *offset)
  {
    ...
    _type = TypeString;
    _value.asString =
      new std::string (XmlRpcUtil::
		       xmlDecode (valueXml.
				  substr (*offset, valueEnd - *offset)));
  ...
  bool XmlRpcValue::timeFromXml (std::string const &valueXml, int *offset)
  {
    ...
    struct tm t;
    if (sscanf
	(stime.c_str (), "%4d%2d%2dT%2d:%2d:%2d", &t.tm_year, &t.tm_mon,
  ...
  std::string XmlRpcValue::binaryToXml ()const
  {
    ...
    int iostatus = 0;
      ...
      encoder.put (_value.asBinary->begin (), _value.asBinary->end (), ins,
  ...
  bool XmlRpcValue::structFromXml (std::string const &valueXml, int *offset)
  {
	...
	const std::string name =
	  XmlRpcUtil::parseTag (NAME_TAG, valueXml, offset);
	// value
  ...
  std::ostream & XmlRpcValue::write (std::ostream & os) const
  {
    switch (_type)
      ...
      default:
	break;
	case TypeBoolean:os << _value.asBool;
	break;
	case TypeInt:os << _value.asInt;
	break;
	case TypeDouble:os << _value.asDouble;
	break;
	case TypeString:os << *_value.asString;
	break;
	case TypeDateTime:
	{
	  struct tm *t = _value.asTime;
      ...
      case TypeStruct:
	{
	  os << '[';
	  ValueStruct::const_iterator it;
	  for (it = _value.asStruct->begin (); it != _value.asStruct->end ();
	       ++it)
	    {
