...
void
XmlRpcUtil::log (int level, const char *fmt, ...)
{
...
void
XmlRpcUtil::error (const char *fmt, ...)
{
  va_list va;
  va_start (va, fmt);
  char buf[1024];
...
rawEntity[] = { '<', '>', '&', '\'', '\"', 0 };
static const char *
xmlEntity[] = { "lt;", "gt;", "amp;", "apos;", "quot;", 0 };
static const int
xmlEntLen[] = { 3, 3, 4, 5, 5 };
  ...
  while (iAmp != iSize)
      ...
      if (encoded[iAmp] == AMP && iAmp + 1 < iSize)
	{
	  ...
	  for (iEntity = 0; xmlEntity[iEntity] != 0; ++iEntity)
	    ...
	    if (strncmp
		(ens + iAmp + 1, xmlEntity[iEntity], xmlEntLen[iEntity]) == 0)
	      {
