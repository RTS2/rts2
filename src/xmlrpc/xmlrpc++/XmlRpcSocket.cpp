...
bool XmlRpcSocket::setReuseAddr (int fd)
{
  ...
  int
    sflag = 1;
  return (setsockopt
	  (fd, SOL_SOCKET, SO_REUSEADDR, (const char *) &sflag,
	   sizeof (sflag)) == 0);
}
...
{
  const int
    READ_SIZE = 4096;		// Number of bytes to attempt to read at a time
  char
    readBuf[READ_SIZE];
...
#endif
      XmlRpcUtil::log (5, "XmlRpcSocket::nbWrite: send/write returned %d.",
		       n);
