...
XmlRpcServerMethod *
XmlRpcServer::findMethod (const std::string & name) const const
{
...
bool XmlRpcServer::bindAndListen (int port, int backlog /*= 5*/ )
{
  ...
  if (fd < 0)
    {
      XmlRpcUtil::
	error ("XmlRpcServer::bindAndListen: Could not create socket (%s).",
...
XmlRpcServer::acceptConnection ()
{
  ...
  if (s < 0)
  ...
  else				// Notify the dispatcher to listen for input on this source when we are in work()
    {
      ...
      _disp.addSource (this->createConnection (s),
...
XmlRpcServer::listMethods (XmlRpcValue & result)
{
  ...
  for (MethodMap::iterator it = _methods.begin (); it != _methods.end ();
       ++it)
    result[i++] = it->first;
