...
inline int
ObjectCheck::is_above_horizont (double ha, double dec, double ra1, double dec1, double ra2, double dec2)
{
...
ObjectCheck::is_good (double st, double ra, double dec, int hardness)
{
  ...
  for (Iter1 = horizont.begin(); Iter1!= horizont.end(); Iter1++)
  {
     if (Iter1->ra > ha)
     {
	return is_above_horizont (ha, dec, last_ra, last_dec, Iter1->ra, Iter1->dec);
     }
     last_ra = Iter1->ra;
     last_dec = Iter1->dec;
  }
  ...
  return is_above_horizont (ha, dec, last_ra, last_dec, Iter1->ra + 24.0, Iter1->dec);
}
