 ...
 * Copyright (C) 2011 Lee Hicks, adaptations for Astrohaven Domes
 ...
 * serial_read_byte(..)
 ******************************************************************************
 * Return single byte read from serial port. Wait until a character is received
 * or a time of timeout milliseconds has expired.
 * Arguments:
 *   sd: open serial port's file descriptor.
 *   timeout_ms: timeout in milli seconds.
 *   buf: pointer to char variable
 * Return:
 *   one of the TTY_ERROR error codes TTY_OK (0), TTY_SELECT_ERROR, TTY_TIME_OUT
 *****************************************************************************/
 int
 serial_read_byte(int sd, int timeout_ms, byte * buf)
 {
   return serial_read_bytes(sd, timeout_ms, buf, 1); 
 }
 ...
 * serial_read_bytes(..)
 ******************************************************************************
 * Return byte(s) read from serial port. Wait until a character is received or
 * a time of timeout milliseconds has expired.
 * Arguments:
 *   sd: open serial port's file descriptor.
 *   timeout_ms: timeout in milli seconds.
 *   buf: pointer to char variable
 *   read_count: integer of how many chars you wish to read
 * Return:
 *   one of the TTY_ERROR error codes TTY_OK (0), TTY_SELECT_ERROR, TTY_TIME_OUT
 *****************************************************************************/
 int
 serial_read_bytes(int sd, int timeout_ms, byte * buf, int read_count)
 {
    int status = serial_timeout(sd, timeout_ms);

    if(status == TTY_OK)
    {
        read(sd, buf, read_count);
        return TTY_OK;
    }
    else
    {
            ...
            int parity, int stop_bits)
{
  ...
  tser_dev *ser_d = (tser_dev *)realloc(NULL, sizeof(tser_dev));
