...
void close_undefined( float setpoint, int interimState, int targetState, int testRun)
{
  int    ret ;
...
void *move_door( void *value)
{
  ...
  while( 1==1) {
	...
	open_close( SETPOINT_CLOSE_DOOR, SETPOINT_CLOSE_DOOR_SLOW, DS_START_CLOSE, DS_RUNNING_CLOSE, test_ssd, SLEEP_CLOSE_DOOR) ;
      }
