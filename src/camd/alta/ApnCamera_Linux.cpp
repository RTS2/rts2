  ...
  if ((DatumA != 0x8086) || (DatumB != 0x1F2F))
    {
      ...
      sprintf (szMsg, "read_Present FAILED.  DatumA:  0x%X   DatumB:  0x%X",
	       DatumA, DatumB);
...
bool
CApnCamera::BufferImage (char *bufferName)
{
...
//    pImageData = (unsigned short *)CCD_locate_buffer(bufferName, 2 , cols, rows, hbin, vbin );
  ...
  pImageData =
    (unsigned short *) malloc ((rows / hbin) * (cols / vbin) * 2 + 4);
  if (pImageData == NULL)
    {
      return 0;
    }
