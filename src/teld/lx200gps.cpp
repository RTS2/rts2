 ...
 *                    Lee Hicks <lee.lorenzo@gmail.com>
 *
...
/*!
 * @file Driver for Meade LX200 GPS protocol based telescope.
 * 
 * Based on the LX200 generic driver. See lx200.cpp for details.
 *
 * @author Matthew Thompson, Lee Hicks
 */
 ...
 */
int
LX200::tel_read (char *buf, int count)
{
	int readed;
	...
	for (readed = 0; readed < count; readed++)
	{
		...
		logStream (MESSAGE_DEBUG) << "LX200 tel_read: read " << buf[read] <<
			sendLog;
		#endif
	}
	return readed;
 ...
 */
int
LX200::tel_read_hash (char *buf, int count)
{
	int readed;
	buf[0] = 0;
	...
	if (buf[readed] == '#')
		buf[readed] = 0;
	logStream (MESSAGE_DEBUG) << "LX200 tel_read_hash: Hash-read: " << buf <<
		sendLog;
	return readed;
}
...
int
LX200::tel_write (const char *buf, int count)
{
...
int
LX200::tel_write_read (const char *wbuf, int wcount, char *rbuf, int rcount)
{
	int tmp_rcount;
	char *buf;
	...
	if (tmp_rcount > 0)
	{
		...
		buf[rcount] = 0;
		logStream (MESSAGE_DEBUG) << "LX200 tel_write_read: read " <<
			tmp_rcount << " " << buf << sendLog;
		free (buf);
	}
	else
	{
		logStream (MESSAGE_DEBUG) << "LX200 tel_write_read: read returns " <<
			tmp_rcount << sendLog;
	}
 ...
 */
int
LX200::tel_write_read_hash (const char *wbuf, int wcount, char *rbuf, int rcount)
{
	int tmp_rcount;
 ...
 */
int
LX200::tel_read_hms (double *hmsptr, const char *command)
{
	char wbuf[11];
 ...
 */
int
LX200::tel_rep_write (char *command)
{
	int count;
	char retstr;
 ...
 */
void
LX200::tel_normalize (double *ra, double *dec)
{
	if (*ra < 0)
								 //normalize ra
 ...
 */
int
LX200::tel_write_dec (double dec)
{
	char command[15];
	int h, m, s;
...
LX200::LX200 (int in_argc, char **in_argv):Telescope (in_argc,
in_argv)
{
...
int
LX200::processOption (int in_opt)
{
	switch (in_opt)
	{
		case 'f':
 ...
 */
int
LX200::tel_set_rate (char new_rate)
{
	char command[6];
...
int
LX200::telescope_start_move (char direction)
{
	char command[6];
...
int
LX200::telescope_stop_move (char direction)
{
	char command[6];
 ...
 */
int
LX200::tel_slew_to (double ra, double dec)
{
	char retstr;
 ...
 */
int
LX200::tel_check_coords (double ra, double dec)
{
	// ADDED BY JF
	double JD;
