...
struct forwarders_struct
{
  int sock;
  struct forwarders_struct *next;
}
forwarders;
     ...
     int port;			/* Port number */
{
  ...
  printf ("opening fw_port %i\n", port);

  if ((sock = socket (AF_INET, SOCK_STREAM, IPPROTO_TCP)) < 0)
    return (serr (sock, "fw_server(): socket."));
  if (setsockopt
  ...
  if (bind (sock, (struct sockaddr *) &s_in, sizeof (s_in)) == -1)
    {
      printf ("fw_bind() errno=%i\n", errno);
      perror ("fw_bind()");
      serr (sock, "fw_server(): bind.");
      exit (1);
    }
  if (listen (sock, 5))
    return (serr (sock, "fw_server(): listen."));

  /* Set socket to non-blocking, so accept will not block */
  fcntl (sock, F_SETFL, O_NONBLOCK);

  serr (0, "fw_server listening");
  return (sock);
}
...
{
  fprintf (s, "PKT INFO:    Received: LT %s", ctime (&tloc));
     ...
     char **argv;
{
  ...
  int listen_socket;		/* socket to listen on */
  struct forwarders_struct *fwd;	/* forwarders */

  int fw_port;

  fd_set read_fd;

  port = 5159;			/* Get the cmdline value for the port number */
  fw_port = 60023;
 ...
 * The return value is the sock descriptor. */
  if ((listen_socket = fw_server (hostname, fw_port)) == -1)
    {
      printf ("cannot open listen socket");
      exit (EXIT_FAILURE);
    }
      ...
      else
	{
	  ...
	  if (FD_ISSET (listen_socket, &read_fd))
	    {
	      ...
	      int res = sizeof (client_side);

	      printf ("is set: %i\n", listen_socket);
	      fflush (stdout);

	      fwd = &forwarders;
	      while (fwd->sock != -1 && fwd->next)
		fwd = fwd->next;
	      if (fwd->sock != -1 && fwd->next == NULL)
		{
		  fwd->next =
		    (struct forwarders_struct *)
		    malloc (sizeof (struct forwarders_struct));
		  fwd = fwd->next;
		  fwd->next = NULL;
		  fwd->sock = -1;
		}
	      if ((fwd->sock =
		   accept (listen_socket, (struct sockaddr *) &client_side,
			   &res)) == -1)
		{
		  perror ("fw_accept");
		}
		...
		{
		  fcntl (fwd->sock, F_SETFL, O_NONBLOCK);
		  printf ("accept new connection on %i from %s:%i\n",
			  fwd->sock, inet_ntoa (client_side.sin_addr),
			  ntohs (client_side.sin_port));
		  fflush (stdout);
		}
	    }

	  for (fwd = &forwarders; fwd; fwd = fwd->next)
	    {
	      if (fwd->sock != -1 && FD_ISSET (fwd->sock, &read_fd))
		{
		  char c[1000];
		  int b = read (fwd->sock, &c, 999);
		  if (b == 0)
		    {
		      printf ("closing %i\n", fwd->sock);
		      close (fwd->sock);
	  ...
	  if (FD_ISSET (inetsd, &read_fd)
	      ...
	      while (fwd)
		{
		  if (fwd->sock != -1)
		    {
		      if (write (fwd->sock, lbuf, bytes) != bytes)
			{
			  close (fwd->sock);
			  fwd->sock = -1;
			}
