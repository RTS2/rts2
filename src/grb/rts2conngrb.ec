...
Rts2ConnGrb::addGcnPoint (int grb_id, int grb_seqn, int grb_type, double grb_ra, double grb_dec, bool grb_is_grb, time_t *grb_date, float grb_errorbox)
{
  ...
  if (sqlca.sqlcode == ECPG_NOT_FOUND)
  {
    ...
    d_tar_name.len = sprintf (d_tar_name.arr, "GRB %02d%02d%02d.%4.3f. GCN #%i (type %i)", 
      ...
      d_grb_id, d_grb_type);
    d_tar_comment.len = sprintf (d_tar_comment.arr, "Generated by GRBD for event %d-%d-%dT%d:%d:%d",
      grb_broken_time->tm_year + 1900, grb_broken_time->tm_mon + 1, grb_broken_time->tm_mday,
      grb_broken_time->tm_hour, grb_broken_time->tm_min, grb_broken_time->tm_sec);
    EXEC SQL
    SELECT
      nextval ('grb_tar_id')
    INTO
