 ...
 *		filter criteria, etc) must be added to the "sites.cfg" file
 *		at the GCN end.
 *
 ...
 *
 *		NET PACKING FORMAT:  The storage/usage of the IP Number and the ????
 *		alre also platform-dependent.  On your site's platform, you may need to make
...
char *version = "socket_demo     Ver: 3.29   23 Mar 05";
...
#define TYPE_SWIFT_UVOT_DBURST_SRC       72	// SWIFT UVOT DarkBurst message (aka Neighbor, aka GeNie)
...
#define TYPE_SWIFT_UVOT_DBURST_PROC_SRC  79	// SWIFT UVOT DarkBurst processed mesg (aka Neighbor, aka GeNie)
...
#		endif
		...
		case TYPE_IPN_SEG_SRC:	// Not implimented for sockets yet
		  ...
		  pr_ipn_seg (lbuf, lg);
		  break;
		  //case TYPE_SAX_WFC_ALERT:          no longer available
		...
		case TYPE_SWIFT_BAT_TRANS:	// 84  // SWIFT BAT Transient Position message
     ...
     FILE *s;			/* Stream to print it to */
{
  ...
  if (id < 99999)
    {
      fprintf (s,
	       "   This Notice is generated as a result of an uploaded TOO -- this is not a normal BAT-detected burst.\n");
      fprintf (s,
	       "   As a result of this TOO, only the GRB_RA/_DEC, SUN, MOON, and GAL_/ECL_COORDS fields will be valid!\n");
    }
  if (lbuf[TRIGGER_ID] & 0x00000010)
    fprintf (s,
	     "   This is an image trigger.    (The RATE_SIGNIF & BKG_{INTEN, TIME, DUR} are undefined.)\n");
  else
  ...
  if (lbuf[TRIGGER_ID] & 0x00000040)
    fprintf (s,
	     "   This trigger occurred during a high background rate interval.\n");
  if (lbuf[TRIGGER_ID] & 0x00000080)
    fprintf (s,
	     "   Since the IMAGE_SIGNIF is less than 7 sigma, this is a questionable detection.\n");
  if (lbuf[TRIGGER_ID] & 0x00000020)
    fprintf (s,
	     "   This is a RETRACTION of a previous notice that identified this as a GRB -- it is NOT a GRB.\n");
  if (lbuf[MISC] & 0x10000000)
    {
      fprintf (s,
	       "   Since all the BAT_Pos_Ack & BAT_LC & BAT_ScaledMap messages were missing in the real-time TDRSS stream,\n");
      fprintf (s,
	       "   this Notice was reconstructed from the Malindi-pass ground-generated BAT_Pos SERS message.\n");
    }
  if (lbuf[MISC] & 0x20000000)
    {
      fprintf (s,
	       "   Since the real-time TDRSS version of the BAT_Pos_Ack message was missing in the TDRSS stream,\n");
      fprintf (s,
	       "   this Notice was reconstructed from pieces of a LightCurve or ScaledMap message.\n");
    }
  if (lbuf[MISC] & 0x40000000)
     ...
     FILE *s;			/* Stream to print it to */
{
  ...
  if (lbuf[MISC] & 0x08000000)
    fprintf (s,
	     "   The 1st packet (of 3) was missing in the lightcurve data stream.\n");
  if (lbuf[MISC] & 0x04000000)
    fprintf (s,
	     "   The 2nd packet (of 3) was missing in the lightcurve data stream.\n");
  if (lbuf[MISC] & 0x02000000)
    fprintf (s,
	     "   The 3rd packet (of 3) was missing in the lightcurve data stream.\n");
  if (lbuf[MISC] & 0x40000000)
    fprintf (s, "   This Notice was ground-reprocessed from flight-data.\n");
  if (lbuf[MISC] & 0x80000000)
    fprintf (s,
  ...
  if (lbuf[18] & 0x2)
    {
      ...
      fprintf (s,
	       "   The S/C has yet to make its decision about safe to slew or not.\n");
    }
  else
     ...
     FILE *s;			/* Stream to print it to */
{
  ...
  if (id < 100000)
    {
      fprintf (s,
	       "   This Notice is generated as a result of an uploaded TOO -- this is not a normal BAT-detected burst.\n");
      fprintf (s,
	       "   As a result of this TOO, only the GRB_RA/_DEC fields will be valid!\n");
    }
  if (lbuf[18] == 0)		// The SC reply
  ...
  else if (lbuf[18] == 1)
    {
      ...
      fprintf (s,
	       "   There maybe a delayed slew or no slew at all; see WAIT_TIME.\n");
    }
  else if (lbuf[18] == 2)
    {
      fprintf (s,
     ...
     FILE *s;			/* Stream to print it to */
{
  ...
  if (lbuf[MISC] & 0x08000000)
    fprintf (s,
	     "   The 1st packet (of 3) was missing in the spectrum data stream.\n");
  if (lbuf[MISC] & 0x04000000)
    fprintf (s,
	     "   The 2nd packet (of 3) was missing in the spectrum data stream.\n");
  if (lbuf[MISC] & 0x02000000)
    fprintf (s,
	     "   The 3rd packet (of 3) was missing in the spectrum data stream.\n");
  if (lbuf[MISC] & 0x40000000)
    fprintf (s,
	     "   This is a ground-generated Notice -- not flight-generated.\n");
  if (lbuf[MISC] & 0x80000000)
     ...
     FILE *s;			/* Stream to print it to */
{
  ...
  if (lbuf[MISC] & 0x08000000)
    fprintf (s,
	     "   The 1st packet (of 3) was missing in the image data stream.\n");
  if (lbuf[MISC] & 0x04000000)
    fprintf (s,
	     "   The 2nd packet (of 3) was missing in the image data stream.\n");
  if (lbuf[MISC] & 0x02000000)
    fprintf (s,
	     "   The 3rd packet (of 3) was missing in the image data stream.\n");
  if (lbuf[MISC] & 0x40000000)
    fprintf (s,
	     "   This is a ground-generated Notice -- not flight-generated.\n");
  if (lbuf[MISC] & 0x80000000)
     ...
     FILE *s;			/* Stream to print it to */
{
  ...
  fprintf (s, "PKT INFO:    Received: LT %s", ctime ((time_t *) & tloc));
  fprintf (s, "   Type= %d   SN= %d    Swift-UVOT DarkBurst\n",
	   lbuf[PKT_TYPE], lbuf[PKT_SERNUM]);
  fprintf (s, "   Hop_cnt=  %d\n", lbuf[PKT_HOP_CNT]);
  fprintf (s, "   PKT_SOD=  %.2f [sec]   delta=%.2f [sec]\n",
	   lbuf[PKT_SOD] / 100.0, here_sod - lbuf[PKT_SOD] / 100.0);
     ...
     FILE *s;			/* Stream to print it to */
{
  ...
  fprintf (s, "PKT INFO:    Received: LT %s", ctime ((time_t *) & tloc));
  fprintf (s, "   Type= %d   SN= %d    Swift-UVOT FindingChart\n",
	   lbuf[PKT_TYPE], lbuf[PKT_SERNUM]);
  fprintf (s, "   Hop_cnt=  %d\n", lbuf[PKT_HOP_CNT]);
  fprintf (s, "   PKT_SOD=  %.2f [sec]   delta=%.2f [sec]\n",
	   lbuf[PKT_SOD] / 100.0, here_sod - lbuf[PKT_SOD] / 100.0);
