...
EOT;
		...
		function s2deg ($s, $mul) {
			...
			$out = $mul * $res;
			$mul = $out < 0 ? -$mul / 60.0 : $mul / 60.0;
			$res = '';
			for ($i++; $i < $len; $i++) {
				$c = $s[$i];
				if ($c > '9' || $c < '0') {
					$out += abs(floatval($res)) * $mul;
					$mul /= 60.0;
					$res = '';
				}
				else
		...
		function deg2s ($d) {
			...
			$m = floor($d);
			$s = ($d - $m) * 60.0;
			printf ("$sgn%d&deg;%02d'%02.1f''", $h, $m, $s);
		}
			...
			function print_table () {
				...
				while ($r < $limit && ($row = pg_fetch_row ($this->res, $r))) {
					...
					for ($i = 0; $i < pg_numfields($this->res); $i++) {
						...
						else if ($fname == 'tar_ra') {
							echo deg2h ($row[$i]) . " ($row[$i]) ";

						}
						else if ($fname == 'tar_dec') {
							echo deg2s ($row[$i]) . " ($row[$i]) ";
						}
			...
			function print_form () {
				...
				while ($row = pg_fetch_row ($this->res)) {
					...
					for ($i = 0; $i < pg_numfields($this->res); $i++) {
						...
						"\t\t</td><td>\n" . ($_SESSION['authorized'] && array_key_exists($name, $fields_writable) ? "<input type='text' name='$name' value='$row[$i]'></input>" : "$row[$i]") . "\n\t</td>\n\t</tr>\n";
					}
					echo "</table>\n\t";
					if ($_SESSION['authorized']) 
